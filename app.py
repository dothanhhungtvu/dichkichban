...
with st.sidebar:
    ...
    st.markdown("---")
    target_language = st.radio("🌐 Chọn ngôn ngữ đích:", ["Tiếng Việt", "Tiếng Anh"], horizontal=True)
...
def translate_script(kịch_bản, api_key, model, target_language="Tiếng Việt"):
    ...
    try:
        ...
        lang_instruction = "sang tiếng Việt" if target_language == "Tiếng Việt" else "sang tiếng Anh"
        
        system_prompt = (
            "Bạn là một biên dịch viên chuyên lồng tiếng phim hoạt hình trẻ em. "
            f"Hãy dịch đoạn hội thoại kịch bản nguồn sau {lang_instruction}. "
...
with col2:
    header_title = "🇻🇳 Kịch Bản Tiếng Việt (Output)" if target_language == "Tiếng Việt" else "🇬🇧 Kịch Bản Tiếng Anh (Output)"
    st.subheader(header_title, divider="rainbow")
    trans_placeholder = st.empty()

# Nút bấm trung tâm (đặt ngoài cột để full width)
...
# Render các text area vào placeholder (xử lý sau cùng để tránh StreamlitAPIException)
clean_placeholder.text_area(
